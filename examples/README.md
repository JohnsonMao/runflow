...
|------------------|-------------|
| `hello-flow.yaml` | Minimal flow: one command step. |
...
| `js-file-flow.yaml` | JS step with `file: ./step.js` (path relative to flow file). |
| `custom-handler/` | Custom step type via `runflow.config.mjs` and handler module; run with `--config` from this dir. |

Helper files: `params.json` is used by `params-schema-flow.yaml`; `step.js` is used by `js-file-flow.yaml`.
