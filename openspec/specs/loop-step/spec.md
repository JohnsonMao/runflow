...
#### Scenario: Body run in DAG order each iteration; done as nextSteps on normal completion
...
- **AND** after iterations the loop step returns `nextSteps: [C]` (done); the executor continues with step C
...
- **AND** done is **not** run; the loop step completes with `nextSteps: [out]`; the executor continues with step `out`
...
#### Scenario: Body and done reference same-flow steps

- **WHEN** a flow has a loop step with `body: [A, B]` and `done: [C]`
- **THEN** step ids A, B, and C SHALL exist in the same flow definition
- **AND** the executor SHALL run body steps A, B as a sub-graph each iteration and use C as nextSteps on normal completion
